...
//!
//! The candidate lists are ordered best first; the caller walks them and takes
//! the first name the GStreamer registry knows.
...
pub fn decoder_candidates(codec: &str, sw_only: bool) -> &'static [&'static CStr] {
    ...
    #[cfg(target_os = "macos")]
    {
        // HEVC on macOS uses avdec_h265, not vtdec: vtdec ignores
        // sps_max_num_reorder_pics and adds output latency. Revert to vtdec once
        // the GStreamer bug is fixed.
...
fn as_str<'a>(p: *const c_char) -> &'a str {
    if p.is_null() {
        return "";
    }
...
/// # Safety
/// `codec` is a NUL-terminated string or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_parser_for(codec: *const c_char) -> *const c_char {
    parser_for(as_str(codec)).as_ptr()
}

/// # Safety
/// `codec` is a NUL-terminated string or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_caps_for(codec: *const c_char) -> *const c_char {
    caps_for(as_str(codec)).as_ptr()
}

/// # Safety
/// `codec` is a NUL-terminated string or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_sw_decoder_for(codec: *const c_char) -> *const c_char {
    sw_decoder_for(as_str(codec)).as_ptr()
}

/// # Safety
/// `name` is a NUL-terminated string or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_is_hw_decoder(name: *const c_char) -> bool {
    is_hw_decoder(as_str(name))
}

/// Writes up to `cap` candidate names into `out` and returns how many. The
/// pointers are static and outlive the caller.
///
/// # Safety
/// `codec` is a NUL-terminated string or null; `out` holds `cap` pointers.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_decoder_candidates(
    codec: *const c_char,
    sw_only: bool,
    out: *mut *const c_char,
    cap: usize,
) -> usize {
    let names = decoder_candidates(as_str(codec), sw_only);
    let n = names.len().min(cap);
    for (i, name) in names.iter().take(n).enumerate() {
        unsafe { *out.add(i) = name.as_ptr() };
    }
    n
}
...
/// The whole pipeline for `codec` through `decoder`. Two queues: ahead of the
/// decoder non-leaky, because a stateless hardware decoder needs every frame for
/// its reference chain; behind it leaky, so a slow sink drops instead of stalling.
...
/// # Safety
/// `codec` and `decoder` are NUL-terminated strings; the result is freed with
/// `cp_string_free`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_pipeline_desc(
    codec: *const c_char,
    decoder: *const c_char,
    with_cal: bool,
) -> *mut c_char {
    let sink_env = std::env::var("LIVI_GST_SINK").unwrap_or_default();
    let desc = pipeline_desc(
        as_str(codec),
        as_str(decoder),
        if with_cal { calibration() } else { "" },
        Some(sink_env.as_str()),
    );
    match std::ffi::CString::new(desc) {
        Ok(s) => s.into_raw(),
        Err(_) => core::ptr::null_mut(),
    }
}

/// True where the platform has a colour-calibration pass.
#[unsafe(no_mangle)]
pub extern "C" fn cp_has_calibration() -> bool {
    !calibration().is_empty()
}

/// # Safety
/// `s` comes from `cp_pipeline_desc`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn cp_string_free(s: *mut c_char) {
    if !s.is_null() {
...
mod tests {
    ...
    fn the_c_entry_points_answer_like_the_safe_ones() {
        let codec = c"h265";
        assert_eq!(unsafe { CStr::from_ptr(cp_parser_for(codec.as_ptr())) }, c"h265parse");
        assert_eq!(unsafe { CStr::from_ptr(cp_sw_decoder_for(codec.as_ptr())) }, c"avdec_h265");
        assert!(unsafe { cp_is_hw_decoder(c"vtdec".as_ptr()) });
        assert!(!unsafe { cp_is_hw_decoder(c"avdec_h264".as_ptr()) });

        let mut out = [core::ptr::null(); 8];
        let n = unsafe { cp_decoder_candidates(codec.as_ptr(), false, out.as_mut_ptr(), out.len()) };
        assert_eq!(n, decoder_candidates("h265", false).len());
        assert_eq!(unsafe { CStr::from_ptr(out[0]) }, decoder_candidates("h265", false)[0]);
    }

    #[test]
    fn a_null_codec_is_read_as_the_default() {
        assert_eq!(unsafe { CStr::from_ptr(cp_parser_for(core::ptr::null())) }, c"h264parse");
        assert!(!unsafe { cp_is_hw_decoder(core::ptr::null()) });
    }

    #[test]
    fn a_short_buffer_takes_only_what_fits() {
        let mut out = [core::ptr::null(); 1];
        let n = unsafe { cp_decoder_candidates(c"h265".as_ptr(), false, out.as_mut_ptr(), 1) };
        assert_eq!(n, 1);
        assert_eq!(unsafe { CStr::from_ptr(out[0]) }, decoder_candidates("h265", false)[0]);
    }
...
mod pipeline_tests {
    ...
    #[test]
    fn the_c_entry_point_answers_like_the_safe_one() {
        let raw = unsafe { cp_pipeline_desc(c"h264".as_ptr(), c"avdec_h264".as_ptr(), false) };
        assert!(!raw.is_null());
        let text = unsafe { CStr::from_ptr(raw) }.to_str().unwrap().to_owned();
        unsafe { cp_string_free(raw) };

        assert!(text.contains("avdec_h264 name=dec"));
        assert!(!text.contains("glshader"));
        unsafe { cp_string_free(core::ptr::null_mut()) };
    }
}
