...
export class TlsBridge extends Duplex {
   ...
   * Call this when an SSL_HANDSHAKE frame arrives (or when an encrypted
   * frame payload is extracted from the AA layer).
   */
 ...
 *
 * Uses tls.connect({ socket: bridge }) — the only reliable way to start a
 * TLS client handshake over a custom Duplex stream in Node.js 18+.
 * new tls.TLSSocket(duplex, { isServer: false }) + write(empty) / emit('connect')
 * are unreliable because TLSSocket only auto-starts the handshake for real
 * net.Socket instances, not plain Duplex streams.
 *
 * @param certPem  PEM string for client certificate (can be self-signed)
 * @param keyPem   PEM string for the private key
 * @param send     callback invoked with raw TLS bytes to be wrapped and sent
 * @returns        { tlsSocket, bridge }
 *                 - tlsSocket: write cleartext messages here, read decrypted data via 'data' event
 *                 - bridge: call bridge.injectBytes() when TLS bytes arrive from phone
 */
export function createTlsClient(
  ...
  // tls.connect({ socket }) automatically initiates the TLS client handshake.
  // - HU is TLS client, phone is TLS server (SSL_set_connect_state in aasdk).
  // - The phone sends CertificateRequest → we must present the canonical HU cert.
  // - SSL_VERIFY_NONE on both sides: certs are used for encryption identity,
  //   not for CA chain validation.
  // - TLS 1.2 only: aasdk use OpenSSL without a version pin, but
  //   Android Auto's on-phone TLS implementation is TLS 1.2.
