...
export class MicChannel extends EventEmitter {
  ...
  handleMessage(msgId: number, payload: Buffer, _frame: RawFrame): void {
    switch (msgId) {
      case AV_MSG.SETUP_REQUEST:
        // Setup request on mic channel — handled by Session._handleAVSetupRequest
        // which calls handleSetupRequest below before sending SETUP_RESPONSE.
        break
      ...
      case AV_MSG.AV_MEDIA_ACK:
        // Flow control — phone confirms it received N frames, we may send more.
   ...
   * timestamp prefix the phone-side decoder expects. Respects max_unacked
   * pacing — frames are queued when the phone hasn't ack'd in time.
   */
  pushPcm(buf: Buffer, ts: bigint): void {
    if (!this._open) return
    if (this._unacked >= this._maxUnacked) {
      // Hold; will be drained on next ACK.
      this._pending.push({ ts, data: buf })
      // Cap backlog — drop oldest if pacing is wedged. Keeps memory bounded.
