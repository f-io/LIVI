...
export class VideoChannel extends EventEmitter {
  ...
  handleMessage(msgId: number, payload: Buffer, frame: RawFrame): void {
    switch (msgId) {
      ...
      case AV_MSG.VIDEO_FOCUS_REQUEST: {
        ...
        // }
        //
        // mode=NATIVE means the user pressed Home/Exit on the AA display and
        // expects the head-unit to show its own UI again (think: Android Auto's
        // own Exit button). openauto signals this via /tmp/entityexit; in LIVI
        // we surface a 'host-ui-requested' event up the chain so aaDriver can
        // inject a Command(CommandMapping.requestHostUI) into ProjectionService.
        // The phone keeps the AA session alive — only the visible focus moves.
        //
        // We always reply with VideoFocusIndication(PROJECTED) regardless of
        // what mode the phone asked for: that's what openauto does, and the
        // phone treats this as a "we acknowledge, focus update incoming".
  ...
  private _sendAck(): void {
    ...
    //   optional uint32 ack                  = 2;   // openauto canonical: 1
    //   repeated uint64 receive_timestamp_ns = 3;   // omitted (matches openauto)
    //
    // Earlier this used a hand-written encoding that mis-encoded session_id
    // (truncated to 7 bits — broke at session_id ≥ 128) and emitted timestamps
    // as 8 raw BE bytes inside a length-delimited record (which is not how
    // proto2 repeated-uint64 wire format works at all). The phone tolerated
    // it because session_id matched, but `max_unacked=1` pacing fell over
    // whenever it didn't, causing frame stalls.
