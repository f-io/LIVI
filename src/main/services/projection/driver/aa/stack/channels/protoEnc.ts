...
export function readVarint(buf: Buffer, off: number): [number, number] {
  ...
  while (pos < buf.length) {
    ...
    shift += 7
    if (shift >= 32) {
      // overflow protection — caller shouldn't be using this for true uint64
      // values, but keep going so the offset advances past the trailing bytes
 ...
 *
 * Skips groups (deprecated wire types 3/4); accepts varint (0), fixed64 (1),
 * length-delimited (2), and fixed32 (5). Yields `Buffer`s that are sub-views
 * of `payload` — copy with `Buffer.from(b)` if you need to retain them past
 * the iterator's lifetime.
 *
 * Use:
 *   for (const f of decodeFields(payload)) {
 *     if (f.field === 1 && f.wire === 2) song = f.bytes.toString('utf8')
 *   }
 */
export function* decodeFields(
  ...
  while (off < payload.length) {
    ...
    if (wire === 0) {
      ...
      // groups / unknown — bail out, payload is malformed for our purposes
      return
    }
  }
}
