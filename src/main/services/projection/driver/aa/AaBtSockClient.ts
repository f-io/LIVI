...
export class AaBtSockClient {
  ...
  private request(line: string, timeoutMs = 5000): Promise<unknown> {
    return new Promise((resolve, reject) => {
      ...
      sock.on('end', () => {
        // Server closed without a newline-terminated payload. If we already
        // settled this is harmless; otherwise it's an error.
  ...
  /** Open a long-lived event subscription. Caller gets one notification per
   *  BlueZ device PropertiesChanged on Connected/Paired. Returns a handle
   *  with .close(); caller is expected to call it when done. */
  subscribe(
